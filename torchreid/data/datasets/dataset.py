...
class ImageDataset(Dataset):
    ...
    def __init__(self, train, query, gallery, **kwargs):
        ...
        self.transform1= Compose([Resize((384, 128))])
        self.transform2= Compose([RandomHorizontalFlip()])
        self.transform3= Compose([ToTensor()])
        self.transform4= Compose([normalize])


        self.transform_last=RandomHorizontalFlip(p=1)
    def __getitem__(self, index):
        img_path, pid, camid = self.data[index]
        img = read_image(img_path)

        maskpath="/home/ubuntu/typ/Partialreid/reid-data/occluded-duke/Occluded_Duke/mask_train/"
        maskimgpath=maskpath+img_path.split('/')[-1]
        if os.path.exists(maskimgpath):
            ...
            if self.transform is not None:
                ...
                if img1!=img:
                    flag=1
                img = self.transform3(img1)
                img = self.transform4(img)
                if flag:
                    maskimage = self.transform1(maskimage)
                    maskimage = self.transform_last(maskimage)
                    maskimage=self.transform3(maskimage)
                else:
                    maskimage = self.transform1(maskimage)
                    maskimage=self.transform3(maskimage)

            return img, pid, camid, img_path,maskimage
        else:
            if self.transform is not None:
