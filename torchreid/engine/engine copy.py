...
from torchreid.utils import AverageMeter, visualize_ranked_results, save_checkpoint, save_checkpoint_G, re_ranking, mkdir_if_missing,read_image
...
class Engine(object):
    ...
    def __init__(self, datamanager, model, G, L_cont, optimizer_m, optimizer_G, scheduler_m, optimizer_L,use_gpu=True):
        self.datamanager = datamanager
        self.model = model


        #self.discriminator=nn.Sequential(nn.Linear(1536,512),
        #                                nn.Linear(512,256),
        #                                nn.Linear(256,camid,bias=False))
    ...
    def run(self, save_dir='log', max_epoch=0, start_epoch=0, fixbase_epoch=0, open_layers=None,
        ...
        for epoch in range(start_epoch, max_epoch):
            ...
            if (epoch+1) % 10 == 0:
                ...
                self._save_checkpoint(epoch, rank1, save_dir)
                self._save_checkpoint_G(epoch, rank1, save_dir)
    ...
    def perturb(self, imgs, train_or_test='test'):
        delta = self.G(imgs)
        delta = L_norm(delta, train_or_test)
        # new_imgs = imgs
        delta_3 = torch.cat([delta,delta,delta], 1)
        # print(imgs.size())
        # print("............................................................")
        new_imgs = torch.mul(imgs.cuda() , delta_3[0:imgs.size(0)].cuda())
        for c in range(3):
            new_imgs.data[:,c,:,:] = new_imgs.data[:,c,:,:].clamp(new_imgs.data[:,c,:,:].min(), new_imgs.data[:,c,:,:].max()) # do clamping per channel
        return new_imgs, delta[0:imgs.size(0)]
    ...
    def _save_checkpoint_G(self, epoch, rank1, save_dir, is_best=False):
        save_checkpoint_G({
            'state_dict': self.G.state_dict(),
            'epoch': epoch + 1,
            'rank1': rank1,
            'optimizer_m': self.optimizer_G.state_dict(),
        }, save_dir, is_best=is_best)