...
class ResNet(nn.Module):
    def __init__(self, num_classes, fc_dims=None, loss=None, dropout_p=None,  **kwargs):
        ...
        self.reduction1_half = nn.Sequential(
            nn.Linear(2048, self.half_feature, 1),
            nn.BatchNorm1d(self.half_feature),
            nn.ReLU()
        )
        self.reduction1_half.apply(weights_init_kaiming)

        self.reduction2_half = nn.Sequential(
            nn.Linear(2048, self.half_feature, 1),
            nn.BatchNorm1d(self.half_feature),
            nn.ReLU()
        )
        self.reduction2_half.apply(weights_init_kaiming)
        ...
        self.global_avgpool_half = nn.AdaptiveAvgPool2d((1, 1))
        self.global_maxpool_half = nn.AdaptiveMaxPool2d((1, 1))
        #self.global_maxpool2 = nn.AdaptiveMaxPool2d((1, 1))
        ...
        self.classifier1 = nn.Linear(2048+self.half_feature, num_classes)
        self.classifier2 = nn.Linear(2048+self.half_feature, num_classes)
        self.classifier3 = nn.Linear(4096+self.half_feature*2, num_classes)
    ...
    def forward(self, x, return_featuremaps=False,segmentmask=False,epoch=None):
        if self.training:
            ...
            f1_half=f1[:,:,:int(f1.shape[2]*self.halfpart),:]
            v1_half = self.global_avgpool_half(f1_half)
            v1_half = v1_half.view(v1_half.size(0), -1)
            v1_1_half = self.reduction1_half(v1_half)  # 512
            v1_1=torch.cat([v1_1,v1_1_half],dim=1)
            ...
            f2_half=f2[:,:,:int(f2.shape[2]*self.halfpart),:]
            v2_half = self.global_maxpool_half(f2_half)
            v2_half = v2_half.view(v2_half.size(0), -1)
            v2_1_half = self.reduction2_half(v2_half)
            v2_1=torch.cat([v2_1,v2_1_half],dim=1)
            ...
            f1_new_half=f1[:,:,:int(f1.shape[2]*self.halfpart),:]
            v1_new_half = self.global_avgpool_half(f1_new_half)
            v1_new_half = v1_new_half.view(v1_new_half.size(0), -1)
            v1_1_new_half = self.reduction1_half(v1_new_half)  # 512
            v1_1_new=torch.cat([v1_1_new,v1_1_new_half],dim=1)
            ...
            f2_new_half=f2_new[:,:,:int(f2.shape[2]*self.halfpart),:]
            v2_new_half = self.global_maxpool_half(f2_new_half)
            v2_new_half = v2_new_half.view(v2_new_half.size(0), -1)
            v2_1_new_half = self.reduction2_half(v2_new_half)
            v2_1_new=torch.cat([v2_1_new,v2_1_new_half],dim=1)
