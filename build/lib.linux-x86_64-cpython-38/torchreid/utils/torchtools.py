...
__all__ = ['save_checkpoint', 'save_checkpoint_G', 'load_checkpoint', 'resume_from_checkpoint',
...
def save_checkpoint_G(state, save_dir, is_best=False, remove_module_from_keys=False):
    r"""Saves checkpoint.
    ...
    # save
    epoch = state['epoch']
    fpath = osp.join(save_dir, 'G.pth.tar-' + str(epoch))
    torch.save(state, fpath)
    print('Checkpoint saved to "{}"'.format(fpath))
    if is_best:
        shutil.copy(fpath, osp.join(osp.dirname(fpath), 'G-best.pth.tar'))
