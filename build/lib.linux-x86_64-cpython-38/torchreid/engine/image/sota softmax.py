...
class ImageSoftmaxEngine(Engine):
    ...
    def perturb1(self, imgs, features=None, train_or_test='test', epoch = 60, epsilon = 0.1):
        b = imgs.size()[0]
        imgs_copy = copy.deepcopy(imgs)
        imgs_copy = torch.cat([imgs_copy, imgs_copy], dim = 0)
        mask = torch.ones([b, 1, imgs.size()[2], imgs.size()[3]])
        pos = torch.empty([b, 2])
        rang = torch.empty([b, 2])
        output_pos = self.G(features)
        pos_probs = F.softmax(output_pos, dim=-1)
        # pos_log_probs = F.log_softmax(output_pos, dim=-1)
        # rang_log_probs = F.log_softmax(output_range, dim=-1)
        # pos_entropy = -(pos_log_probs * pos_probs) 
        # rang_entropy = -(rang_log_probs * rang_probs) 
        # pos, output_pos_max = torch.topk(pos_probs, 3, -1, True)
        # rang, output_range_max = torch.topk(rang_probs, 3, -1, True)
        # temp = random.random()
        output_pos_max = pos_probs.multinomial(num_samples=2)
        # pos, output_pos_max = torch.topk(pos_probs, 2, -1, True)
        # output_pos_random = torch.from_numpy(np.random.randint(0,45,size=(40, 1))).cuda()
        # print(pos_probs.shape)
        # for i in range(b):
        #     for j in range(3):
        #         pos[i,j] = pos_probs[i][output_pos_max[i][j]]
        #         # print(pos_log_probs[i][output_pos_max[i]].grad_fn.next_functions[0][0].next_functions[0][0].next_functions[0][0].next_functions[0][0])
        #         rang[i,j] = rang_probs[i][output_range_max[i][j]]
        #         # pos_entro.append(pos_entropy[i][output_pos_max[i]])
        #         # rang_entro.append(rang_entropy[i][output_range_max[i]])
        for i in range(imgs.size()[0]):
            pos[i] = pos_probs[i][output_pos_max[i]]

        # output = torch.cat([output_pos_max, output_pos_random], dim = 1)
        output_range_max = output_pos_max % 4
        output_pos_max = torch.floor(output_pos_max / 4)
        # print(output_range_max)
        # print(output_pos_max)
        for j in range(2):
            row_x = output_pos_max[:,j] / 2
            ...
            for i in range(imgs.size()[0]):
                for attempt in range(10000000000):
                    target_area = output_range_max[i,j] * area / 9
                    ...
                    if length_y < imgs.size()[3] and length_x < imgs.size()[2]:
                        ...
                        break
        # delta1 = 1/(1+ torch.exp(-50 * (delta - 0.3)))
        # delta1 = torch.where(delta > 0.3, 1, 0)
        
        # #delta = L_norm(delta, train_or_test)
        # # new_imgs = imgs
        # delta_3 = torch.cat([delta1,delta1,delta1], 1)
        # # print(imgs.size())
        # # print("............................................................")
        # new_imgs = imgs*delta_3#torch.mul(imgs.cuda() , delta_3[0:imgs.size(0)].cuda())
        # delta.cuda()
        # #for c in range(3):
        ...
        #     np.savetxt("/media/tongji/data/qzf/Paritalreid-1/rang_pro/" +'final.txt', output_range.detach().cpu().numpy())
        return imgs_copy, pos.cuda(), mask.cuda()
    ...
    def train(self, epoch, max_epoch, trainloader, fixbase_epoch=0, open_layers=None, print_freq=10):
        ...
        imgs_pre = torch.zeros([40, 3, 384, 128])
        ...
        self.model.train()
        self.G.train()
        # imgs_fix = torch.zero([80, 3, 384, 128])
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            img_GAN, pos, mask = self.perturb1(imgs, features, 'train', epoch = epoch)
            # img_GAN_dis, _, _, _, _ = self.perturb1(img_GAN.detach(), 'train_1')
            output1, output2, output3 = self.model(img_GAN[0:40], img_GAN[40:], epoch=epoch,segmentmask=maskimages.cuda())
            ...
            # loss_x4 = self._compute_loss(self.criterion1, output4, pids)*1.1
            ...
            img_GAN, pos, mask = self.perturb1(imgs_fix, features, 'train', epoch = epoch)
            output1, output2, _ = self.model(img_GAN[0:40], img_GAN[40:], epoch=epoch,segmentmask=maskimages.cuda())
            ...
            # loss_cont_1 = (loss_controller_1.max() - loss_controller_1) / (torch.std(loss_controller_1) + 1e-5)
            # loss_cont_2 = (loss_controller_2.max() - loss_controller_2) / (torch.std(loss_controller_2) + 1e-5)
            # loss_cont_1 = (loss_controller_1 - loss_controller_1.min()) / (torch.std(loss_controller_1) + 1e-5)
            # loss_cont_2 = (loss_controller_2 - loss_controller_2.min()) / (torch.std(loss_controller_2) + 1e-5)
            loss_cont = torch.stack([loss_controller_1, loss_controller_2], dim = 1)
            loss_cont = (loss_cont.detach() * (pos)).mean() * 50

            self.optimizer_G.zero_grad()
            loss_cont.backward()
            self.optimizer_G.step()

            losses_cont.update(loss_cont.item(),pids.size(0))
