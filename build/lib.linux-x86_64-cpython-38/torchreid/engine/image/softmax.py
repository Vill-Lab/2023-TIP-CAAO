...
def get_masks(im_names, all_masks=None, prob_diff=None):
    ...
    for name in im_names:
        if np.random.uniform() < 0.5:
            ...
            p_d[p_d < 0] = 0
            p = p_d / np.sum(p_d)
            ind = np.random.choice(range(len(p_d)), size = 3, replace = False, p=p)
            
            # ind1 = np.argsort(- p_d)[0]
            # ind2 = np.argsort(- p_d)[1]
            # ind3 = np.argsort(- p_d)[2]
            mask1 = torch.from_numpy(all_masks[ind[0]]).cuda().unsqueeze(0)
            mask2 = torch.from_numpy(all_masks[ind[1]]).cuda().unsqueeze(0)
            mask3 = torch.from_numpy(all_masks[ind[2]]).cuda().unsqueeze(0)
...
class ImageSoftmaxEngine(Engine):
    ...
    def train(self, epoch, max_epoch, trainloader, fixbase_epoch=0, open_layers=None, print_freq=10):
        ...
        self.prob_diff = load_pickle('/home/v-zefanqu/v-zefanqu/CAAO/Paritalreid/log/Occduke_sw_occlusion_0.1range/prob_diff.pkl')
        self.prob_diff = blur_prob_diff(self.prob_diff)
        print(len(self.prob_diff['mirrored'].keys()))
        self.all_masks = load_pickle('/home/v-zefanqu/v-zefanqu/CAAO/Paritalreid/log/Occduke_sw_occlusion_0.1range/all_masks.pkl')
