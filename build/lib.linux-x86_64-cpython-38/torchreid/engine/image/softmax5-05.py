...
class ImageSoftmaxEngine(Engine):
    ...
    def train(self, epoch, max_epoch, trainloader, fixbase_epoch=0, open_layers=None, print_freq=10):
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            loss_x4 = self._compute_loss(self.criterion, output4, pids)

            loss_distill=(loss_kd(output1_new,pids,output1.detach())+loss_kd(output2_new,pids,output2.detach())+loss_kd(output4_new,pids,output3.detach()))#+loss_kd(output4_new,pids,output4.detach()))#+loss_kd(output4_new,pids,output4.detach()))
          
            loss_x = torch.stack([loss_x1, loss_x2,loss_x3,loss_x4], dim=0)
