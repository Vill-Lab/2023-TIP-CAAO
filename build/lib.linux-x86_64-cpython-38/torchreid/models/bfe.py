...
class ResNet(nn.Module):
    def __init__(self, num_classes, fc_dims=None, loss=None, dropout_p=None,  **kwargs):
        ...
        # self.res_part4 = Bottleneck(2048, 512)

        # self.att_module2 = CBAM_Module(512)
        # self.att_module3 = CBAM_Module(1024)
        ...
        #     nn.ReLU()
        # )
        # self.reduction4.apply(weights_init_kaiming)
    ...
    #     # layer40_weight = (self.layer41[0].weight + self.layer42[0].weight + self.layer43[0].weight) / 3
    #     # layer41_weight = (self.layer41[1].weight + self.layer42[1].weight + self.layer43[1].weight) / 3
    #     # layer42_weight = (self.layer41[2].weight + self.layer42[2].weight + self.layer43[2].weight) / 3
    #     # res_part_weight = (self.res_part1.weight + self.res_part2.weight + self.res_part3.weight) / 3
    #     # reduction0_weight = (self.reduction1[0].weight + self.reduction2[0].weight + self.reduction3[0].weight) / 3
    #     # reduction1_weight = (self.reduction1[1].weight + self.reduction2[1].weight + self.reduction3[1].weight) / 3
    #     # classifier_weight = (self.classifier1.weight + self.classifier2.weight + self.classifier3.weight) / 3
    #     # self.layer41[0].weight = layer40_weight
    #     self.layer42.load_state_dict(self.layer41.state_dict())
    #     self.layer43.load_state_dict(self.layer41.state_dict())
        
    #     self.res_part2.load_state_dict(self.res_part1.state_dict())
    #     self.res_part3.load_state_dict(self.res_part1.state_dict())
        
    #     self.reduction2.load_state_dict(self.reduction1.state_dict())
    #     self.reduction3.load_state_dict(self.reduction1.state_dict())
        
    #     self.classifier2.load_state_dict(self.classifier1.state_dict())
    ...
    def forward(self, x, x_2=None, x_3=None, return_featuremaps=False, state='test', segmentmask=False,epoch=None,returnflag=False, update_param = False):
        ...
        if self.training:
            ...
            if return_featuremaps:
                ...
                # f1 = self.res_part1(f1_ori)
                # f2 = self.res_part2(f2_ori)
                # f3 = self.res_part3(f3_ori)

                # # f4 = self.res_part2(f4)
            ...
            if state == 'train':
                ...
                # f3, f4 = self.featuremaps_my(x_3, x_4)
