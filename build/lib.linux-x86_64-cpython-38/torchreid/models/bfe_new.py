...
class ResNet(nn.Module):
    ...
    def mapdrop(self,featuremap):
        mask=[]
        for j in range(outputs.size(0)):
            featmap = outputs[j, ...].detach()
            featmap=torch.sum(featmap,dim=2)
    ...
    def adversarial_erase(self,outputs,width=384,height=128):
        mask=[]
        for j in range(outputs.size(0)):
            am = outputs[j, ...].detach().cpu().numpy()
            am = cv2.resize(am, (width, height))
            ...
            am = np.uint8(np.floor(am))
            am_mask=torch.from_numpy(am)
            
            am_mask=torch.sum(am_mask,dim=2)
            am_mask=am_mask.reshape(width,height)
            maxvalue=am_mask.max()
            am_mask=torch.where(am_mask > 0.7*maxvalue, 0, 1)
            am_mask=am_mask.unsqueeze(dim=0)
            am_mask=torch.cat([am_mask,am_mask,am_mask],dim=0)
            mask.append(am_mask)
            m=np.argmax(am)
            r, c = divmod(m, am.shape[1])
        ...
        mask=mask.cuda()
        return mask
    def featuremaps_adver(self, x,mask=None,segmentmask=False,epoch=None):
        if self.training:
            b = x.size(0)
            x_1 = x.clone()
            x_1= mask*x
    ...
    def forward(self, x, return_featuremaps=False,segmentmask=False,epoch=None):
        ...
        if self.training:
            ...
            mask=self.adversarial_erase(fmap)
            ferase_1,ferase_2,x_new=self.featuremaps_adver(x,mask,segmentmask,epoch=epoch)

            fmap_new=ferase_2.clone()
            mask_new=self.adversarial_erase(fmap_new)
            ferase_1_new,ferase_2_new,_=self.featuremaps_adver(x_new,mask_new,segmentmask,epoch=epoch)
