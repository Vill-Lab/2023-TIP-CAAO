...
class ResNet(nn.Module):
    ...
    def featuremaps_my(self, x,erase_x=None,erase_y=None,erase_x_min=None,erase_y_min=None):
        if self.training:
            b = x.size(0)
            x_1 = x.clone()
            x_1= self.batch_erase(x_1,erase_x,erase_y)
            x_new=x_1.clone()
            x_2 = x.clone()
    ...
    def forward(self, x, return_featuremaps=False,segmentmask=False,epoch=None):
        if self.training:
            ...
            ferase_1,ferase_2,ferase_3,x_new=self.featuremaps_my(x,erase_x,erase_y,erase_x_min,erase_y_min)

            
            fmap_new=ferase_2.clone()
            erase_x_new,erase_y_new,erase_x_newmin,erase_y_newmin=self.attent_erase(fmap_new)
            ferase_1_new,ferase_2_new,ferase_3_new,_=self.featuremaps_my(x_new,erase_x_new,erase_y_new,erase_x_newmin,erase_y_newmin)
