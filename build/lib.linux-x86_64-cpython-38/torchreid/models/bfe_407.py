...
class ResNet(nn.Module):
    ...
    def featuremaps_my(self, x,erase_x=None,erase_y=None,erase_x_min=None,erase_y_min=None,segmentmask=False,epoch=None):
        if self.training:
            b = x.size(0)
            x_1 = x.clone()
            x_1= self.batch_erase(x_1,erase_x,erase_y,segmentmask,epoch)
            x_1_min = x.clone()
            x_1_min= self.batch_erase(x_1_min,erase_x_min,erase_y_min,segmentmask,epoch)
            x_new=x_1.clone()
            x = torch.cat([x, x_1,x_1_min], 0)
    ...
    def attent_erase(self,outputs,width=384,height=128):
        ...
        for j in range(outputs.size(0)):
            ...
            minpoint=np.argmin(am)
            rpoint, cpoint = divmod(minpoint, am.shape[1])
            erase_x_min.append(rpoint)
            erase_y_min.append(cpoint)
    ...
    def forward(self, x, return_featuremaps=False,segmentmask=False,epoch=None):
        if self.training:
            ...
            erase_x,erase_y,erase_x_min,erase_y_min=self.attent_erase(fmap)
            ferase_1,ferase_2,ferase_3,x_new=self.featuremaps_my(x,erase_x,erase_y,erase_x_min,erase_y_min,segmentmask,epoch=epoch)

            
            fmap_new=ferase_2.clone()
            erase_x_new,erase_y_new,erase_x_newmin,erase_y_newmin=self.attent_erase(fmap_new)
            ferase_1_new,ferase_2_new,ferase_3_new,_=self.featuremaps_my(x_new,erase_x_new,erase_y_new,erase_x_newmin,erase_y_newmin,segmentmask,epoch=epoch)
            ...
            v3_new = self.global_maxpool2(f3_new)
            ...
            if self.loss == 'softmax':
                return y1, y2, y3,y4,y1_new, y2_new, y3_new,y4_new
