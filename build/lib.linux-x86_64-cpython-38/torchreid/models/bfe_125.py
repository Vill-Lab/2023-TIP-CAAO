...
class RandomErasing_v2(nn.Module):
    def __init__(self, sl=0.2, sh=0.333, r1=0.5, mean=[0.4914, 0.4822, 0.4465]):
            ...
            for attempt in range(100):
                ...
                if w < img.size()[3] and h < img.size()[2]:
                    ...
                    if y1+w>img.size()[3]:
                        y1=img.size()[3]-w
                    if img.size()[1] == 3:
                        img[i, 0, x1:x1 + h, y1:y1 + w] = random.uniform(0, 1)#self.mean[0]
                        img[i, 1, x1:x1 + h, y1:y1 + w] = random.uniform(0, 1)#self.mean[1]
                        img[i, 2, x1:x1 + h, y1:y1 + w] = random.uniform(0, 1)#self.mean[2]
...
class RandomErasing_v3(nn.Module):
    ...
    # img 32,3,384,128
    def forward(self, img,erase_x,erase_y,fmap):
        fmap_return=fmap.clone()
        masks = np.ones([img.size(0), 4])
        for i in range(img.size(0)):
            for attempt in range(100):
                area = img.size()[2] * img.size()[3]
                ...
                if w < img.size()[3] and h < img.size()[2]:
                    ...
                    if img.size()[1] == 3:
                        ...
                        fmap_return[i,:,x1:x1 + h, y1:y1 + w]=0
                        masks[i] = x1, x1+h, y1, y1+h
                        break
        return img,fmap_return
...
class ResNet(nn.Module):
    ...
    def __init__(self, num_classes, fc_dims=None, loss=None, dropout_p=None,  **kwargs):
        ...
        self.batch_erase = RandomErasing_v3()
        ...
        self.classifier4 = nn.Linear(512*3, num_classes)
    ...
    def featuremaps_my(self, x,activatemap=None,erase_x=None,erase_y=None):
        
        if self.training:
            b = x.size(0)
            x_1 = x.clone()
            x_1,activatemap_erase= self.batch_erase(x_1,erase_x,erase_y,activatemap)

            erase_x2,erase_y2=self.attent_erase_step2(activatemap_erase)
            x_2=x.clone()
            x_2,_= self.batch_erase(x_2,erase_x2,erase_y2,activatemap_erase)
        ...
        if self.training:
            ...
            x_3 = self.layer43(x_3)        
        if self.training:
            return x_1, x_2,x_3, pred_map1,pred_map2
    ...
    def attent_erase(self,outputs,width=384,height=128):
        ...
        for j in range(outputs.size(0)):
            ...
            erase_x.append(r)
            erase_y.append(c)
            activate_map.append(am)

        erase_x=torch.tensor(erase_x).cuda()
        erase_y=torch.tensor(erase_y).cuda()
        activate_map=torch.tensor(activate_map).cuda()
        return erase_x,erase_y,activate_map

    def attent_erase_step2(self,outputs,width=384,height=128):
        erase_x=[]
        erase_y=[]
        for j in range(outputs.size(0)):
            am = outputs[j, ...].detach().cpu().numpy()
    ...
    def forward(self, x, return_featuremaps=False):
        if self.training:
            ...
            erase_x,erase_y,activatemap=self.attent_erase(fmap)

            ferase_1,ferase_2,ferase_3,_,_=self.featuremaps_my(x,activatemap,erase_x,erase_y)
            f1=ferase_1
            f2=ferase_2
            f3=ferase_3
            ...
            f3, pred_map3 = self.att_module3(f3)
